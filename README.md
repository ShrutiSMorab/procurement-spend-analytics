...
>
> All data is **synthetic**, generated by [`data/generate_data.py`](data/generate_data.py) with realistic procurement patterns baked in. No employer or client data is used. Savings figures are **modelled, not realised results**. Companion project: [last-mile delivery analytics](https://github.com/ShrutiSMorab/lastmile-delivery-sql-analytics).
...
### 📄 [**Read the full case study →**](CASE-STUDY.md)  ·  📊 [**View the deck →**](deliverables/Indirect-Spend-Optimisation-Deck.pdf)  ·  🔍 [**Study guide: every query explained →**](STUDY-GUIDE.md)
...
|---|---|---|
| [`01_data_quality_checks.sql`](sql/01_data_quality_checks.sql) | Can we trust the data? | LEFT JOIN + IS NULL, consistency checks |
| [`02_spend_overview.sql`](sql/02_spend_overview.sql) | Spend cube | window % of total, month bucketing |
| [`03_supplier_concentration.sql`](sql/03_supplier_concentration.sql) | Pareto / ABC | cumulative SUM() OVER, classification |
| [`04_price_variance.sql`](sql/04_price_variance.sql) | Same item, different price | MIN/MAX spreads, savings simulation |
| [`05_maverick_spend.sql`](sql/05_maverick_spend.sql) | Off-contract buying | LEFT JOIN as business signal |
| [`06_tail_spend.sql`](sql/06_tail_spend.sql) | Supplier tail | threshold bucketing, opportunity sizing |

Full raw outputs: [`docs/query_outputs.txt`](docs/query_outputs.txt) · Raw data + field dictionary: [`data/spend-raw-data.xlsx`](data/spend-raw-data.xlsx)
...
![Supplier Pareto](assets/pareto-suppliers.png)
...
![Maverick spend by BU](assets/maverick-by-bu.png)
...
[`dashboard/spend_dashboard.xlsx`](dashboard/spend_dashboard.xlsx) — a live, formula-driven workbook (no hardcoded results: 13,500+ SUMIF/SUMIFS formulas over the raw PO lines) with a KPI header and four charts: spend by category, monthly trend, maverick % by BU, top-10 suppliers. Built with [`dashboard/build_dashboard.py`](dashboard/build_dashboard.py).

## 8. Consulting deliverables

| Deliverable | Format |
|---|---|
| Written consulting report — situation, diagnostic, savings bridge, recommendations, roadmap, risks | [PDF](deliverables/Indirect-Spend-Optimisation-Case-Study.pdf) · [Word](deliverables/Indirect-Spend-Optimisation-Case-Study.docx) |
| Executive deck — 11 slides, with speaker notes | [PDF](deliverables/Indirect-Spend-Optimisation-Deck.pdf) · [PowerPoint](deliverables/Indirect-Spend-Optimisation-Deck.pptx) |
...
sqlite3 spend.db                  # then: .read ../sql/02_spend_overview.sql
